...
function SignUpPageContent() {
  ...
  const bootstrapAdminClientRecord = async (
    ...
    const response = await fetch("/api/auth/bootstrap", {
      ...
      body: JSON.stringify({
        companyName,
        displayName: name || accountUser.displayName || "",
      }),
    })
    const payload = (await response.json().catch(() => null)) as { error?: string } | null
    if (!response.ok) {
      throw new Error(payload?.error || "Unable to initialize the admin client roster.")
    }
  }
    ...
    >
      <section className="space-y-4 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className="text-xl font-semibold text-slate-950">Continue with Google</h2>
            <p className="mt-1 text-sm leading-6 text-slate-600">
              Use your Google account to initialize the client roster and preserve your
              business context.
            </p>
          </div>
        </div>
        ...
        <button
          type="button"
          className="w-full text-center text-sm font-semibold text-slate-600 underline-offset-4 transition hover:text-slate-950 hover:underline"
          onClick={() => setShowManualRegistration(true)}
          disabled={loading || handoffLoading}
        >
          Or register manually with email
        </button>
      </section>
      ...
      {showManualRegistration ? (
        <div className="fixed inset-0 z-50 flex items-end bg-slate-950/40 p-3 backdrop-blur-sm sm:items-center sm:justify-center">
          <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[28px] border border-slate-200 bg-white p-5 shadow-2xl sm:p-6">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-slate-600">
                  Enter only the fields needed to initialize your client account record.
                </p>
              </div>
              <button
                type="button"
                className="rounded-full border border-slate-200 px-3 py-1 text-sm font-semibold text-slate-600 transition hover:border-slate-300 hover:text-slate-950"
